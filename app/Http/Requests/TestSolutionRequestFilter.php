...
/**
 * Class TestSolutionRequestFilter
 *
 * @package App\Http\Requests
 */
final class TestSolutionRequestFilter extends RequestFilter
{
    ...
    public function validate(Request $request, Test $test): void
    {
        ...
        if ($user->role < RolesEnum::ROLE_ASSISTANT) {
            if (
                $request->get(self::FIELD_GROUP_ID) &&
                !$user->testSolutions()->where(GroupStudent::ATTR_GROUP_ID, $request->get(self::FIELD_GROUP_ID))->first()
            ) {
                throw ValidationException::withMessages([
                    self::FIELD_GROUP_ID => "Nemáte oprávnění řešit tuhle skupinu.",
                ]);
            }
        ...
        if ($request->get(self::FIELD_GROUP_ID)) {
            $group = $test->groups()->find($request->get(self::FIELD_GROUP_ID));
            if (!$group) {
                throw ValidationException::withMessages([
