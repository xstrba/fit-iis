...
final class Question extends Model implements HasFileTraitInterface
{
    ...
    protected $casts = [
        ...
        self::ATTR_MIN_POINTS => 'float',
        self::ATTR_MAX_POINTS => 'float',
    ];
     ...
     * @return float
     */
    public function getMinPointsAttribute(float $value): float
    {
        if ($this->type === QuestionTypesEnum::OPTIONS_CHECKBOX) {
            ...
            })->map(static function (Option $option): float {
     ...
     * @return float
     */
    public function getMaxPointsAttribute(float $value): float
    {
        if ($this->type === QuestionTypesEnum::OPTIONS_CHECKBOX) {
            return (int)\array_sum(\array_merge([0], $this->options->filter(static function (Option $option): bool {
                return $option->points > 0;
